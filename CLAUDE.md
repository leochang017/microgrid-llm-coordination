...
- **Current position:** Task 17 next (integration test — round-robin should beat no-coordination on Gini). Tasks 0-16 complete — engine main loop runs end-to-end. Simulator is now executable: `python -c "from pathlib import Path; from sim.engine import run; from sim.scenario import load_scenario; from sim.logging import JsonlLogger; from sim.strategies.no_coordination import decide_transfers; s=load_scenario(Path('configs/scenarios/synthetic_smoke.yaml')); lg=JsonlLogger('runs/manual', s.scenario_id); print(run(s, decide_transfers, lg)); lg.close()"` produces a real summary.json.
