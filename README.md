...
and table is re-derived from. `python -m scripts.figures --all` regenerates the whole
results section from them for $0. A `--depth 1` clone skips the history but not these files.
...
```bash
pytest                  # full suite; the CI badge at the top links the current run
